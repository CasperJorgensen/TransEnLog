...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    // Adding new contact
    public void addShoks(Shock shock) {
        SQLiteDatabase db = this.getWritableDatabase();

        ContentValues values = new ContentValues();
        values.put(KEY_TIME, String.valueOf(shock.getTime())); // Contact Name
        values.put(KEY_SHOCKSTRENGTH, shock.getShock()); // Contact Phone Number
    ...
    public Shock getShocks(int id) {
        ...
        Shock shock = new Shock(Integer.parseInt(cursor.getString(0)),
    ...
    public List<Shock> getAllShocks() {
        // Getting All Contacts

        List<Shock> shocksList = new ArrayList<Shock>();
        // Select All Query
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                //Fejlene her er datoen!!     shock.setTime(cursor.getString(1));
                // Midlertidig lÃ¸sning under!

                Date date = new Date();
                shock.setId(Integer.parseInt(cursor.getString(0)));
                shock.setTime(date);


                shock.setShockStrenght(cursor.getLong(2));
                // Adding contact to list
                shocksList.add(shock);
    ...
    public int updateShocks(Shock shocks) {


        SQLiteDatabase db = this.getWritableDatabase();

        ContentValues values = new ContentValues();
        values.put(KEY_TIME, String.valueOf(shocks.getTime()));
    ...
    public void deleteShocks(Shock shocks) {
        SQLiteDatabase db = this.getWritableDatabase();
